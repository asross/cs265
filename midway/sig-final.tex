...
We wrote Python code to simulate how , an LSM tree with a variably sized cache,
memtable, disk layers, and bloom filters performs for an arbitrary sequence of
queries \cite{lsmulator}. In particular, we count disk accesses (the main
performance bottleneck for an LSM tree) as well as statistics about the utility
of each component. So far, results suggest that the same LSM tree architecture
performs very differently under different query distributions (Figure
\ref{fig:lsametree}), and that different LSM tree architectures perform
very differently under the \textit{same} query distribution
%(Figure
%\ref{fig:sameqs-difftree}).

\begin{figure*}[!htb]
\begin{center}
\includegraphics[width=\textwidth]{lsametree.png}
\end{center}
\caption{The same LSM tree architecture performs differently for different
  query distributions. Memtable and cache hits (in green) are fast whereas
  accesses to the layers (in red) are slow. For the Zipf workload, the cache is
  much more useful than the buffer, while the situations are reversed for the
Round-Robin workload. Both are useful for Discover-Decay.}
\label{fig:lsametree}
\end{figure*}
...
%cache and the buffer are useful, but finding the best ratio may require
%optimization.}
