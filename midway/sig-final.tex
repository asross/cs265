...
These results demonstrate (1) there are many qualitatively distinct regimes of optimality in an LSM tree, (2) all components can be important, (3) Monkey consistently outperforms the baseline bloom filter allocation, even on fairly exotic workalods, and (4) due to the possibility of non-convexity, globally optimizing memory allocation might be challenging.

\section{Adaptive Allocation}

But we can try anyway. Our key insight (thanks to Niv Dayan for this helpful
suggestion) is to, during the normal query process, gather local statistics
with low memory footprints in our LSM tree from which we can estimate the
usefulness of each LSM tree component. We measure ``usefulness'' in the number
of IOs we can save by adding an additional unit of memory to that component,
or conversely the number of IOs we incur by removing one. These quantities
can be interepreted as discrete \textit{gradients} of our LSM tree components
with respect to IOs.

\subsection{Stochastic DB Gradient Descent}
...
\subsection{Gradient of the Cache}

\subsection{Gradient of the Bloom Filters}

\subsection{Gradient of the Buffer}

\begin{figure*}[!htb]
\begin{center}
\includegraphics[width=\textwidth]{all-savings.png}
\end{center}
\caption{Light-footprint statistical estimations of the gradient vs. simulated results for cache, bloom filters, and the buffer on three distinct workloads.}
\label{fig:period-lsmulations}
\end{figure*}
